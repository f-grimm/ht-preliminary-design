...
def get_initial_mtow(aircraft: object):
	...
	profile. [p.90-91]
	"""
	fuel_mass = aircraft.sfc * aircraft.power_available * aircraft.duration
	useful_load = fuel_mass + aircraft.payload + aircraft.crew_mass
...
def iterate_first_sizing(aircraft: object):
	...
	fuel_mass = aircraft.sfc * 1.1 * aircraft.hover_power * aircraft.duration
	aircraft.mtow = (empty_weight + fuel_mass + aircraft.payload 
	                 + aircraft.crew_mass)
...
def iterate_second_sizing(aircraft: object):
	...
	rotor = aircraft.main_rotor
	aircraft.advance_ratio = aircraft.flight_speed / rotor.tip_velocity
	thrust = aircraft.get_thrust(drag)
	induced_velocity = rotor.get_induced_velocity(
		aircraft.density, aircraft.flight_speed, aircraft.alpha, thrust)

	# Power calculation
	induced_power = rotor.kappa * thrust * induced_velocity
	profile_power = rotor.get_profile_power(
		aircraft.density, aircraft.advance_ratio)
...
def preliminary_design(aircraft: object, logs=True):
	...
	Requires:
		...
		Tail rotor
	"""
	aircraft.set_mission_segment(1)
	aircraft.mtow = get_initial_mtow(aircraft)

	if logs:
		print(f'Mission segment: 1')
		print(f'Height: {aircraft.height:.2f} m\n-----')
